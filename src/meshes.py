        ...
        if len(obj.data.uv_textures) > 0 :
            for uvChan in range(len(obj.data.uv_textures)):
                if uvChan == 0 :
                    ...
                    obj.data.uv_textures[uvChan].name = "UV{}".format((uvChan+1))
    return {'FINISHED'}
    
def selectUVChannels(objectSelection, uvChan):
    for obj in objectSelection:
...
if __name__ == "__main__":
    ...
    selectUVChannels([o for o in bpy.context.selected_objects if o.type == 'MESH'], 1)